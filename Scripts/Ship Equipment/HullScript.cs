...
public class HullScript : MonoBehaviour {
	...
	public void setHull (HullType hullType) {
		setHull(hullType, getRandomProtection (hullType));
	}
	...
	private int getRandomProtection (HullType hullType) {
		return (int) Mathf.Round(Utils.getRandomValue(hullType.getProtection(), 3));
	}
	...
	public int getAvailableWeaponSlots () {
		return hullType.getAvailableWeaponSlots();
	}
}
